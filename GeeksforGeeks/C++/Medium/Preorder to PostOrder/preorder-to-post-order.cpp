...
class Solution{
public:
    Node *solve(int l,int r,int pre[]){
        if(l>r) return NULL;
        int idx=lower_bound(pre+l+1,pre+r+1,pre[l])-pre;
        Node *cur=newNode(pre[l]);
        cur->left=solve(l+1,idx-1,pre);
        cur->right=solve(idx,r,pre);
    }
